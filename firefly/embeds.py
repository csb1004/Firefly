...
def create_help_embed() -> discord.Embed:
    ...
    fields = [
        ...
        ("/최신 소식 받기", "매일 정리되는 기술 소식을 개인 메시지로 받아."),
        ("/최신 소식 그만", "최신 소식 개인 메시지를 더 받지 않아."),
        ("/최신 소식 상태", "내 구독 상태와 발송 시간을 개인 메시지로 확인해."),
        ("/주제 목록", "최신 소식 주제를 개인 메시지로 확인해."),
...
def create_special_help_embed() -> discord.Embed:
    ...
    fields = [
        ...
        ("/최신 소식 받기 [@유저]", "매일 정리되는 기술 소식을 개인 메시지로 받아. @유저 추가는 특별 사용자만 가능해."),
        ("/최신 소식 그만 [@유저]", "최신 소식 수신을 해제해. 다른 사람 해제는 특별 사용자만 가능해."),
        ("/최신 소식 시간 [09:00]", "최신 소식 발송 시간을 확인하거나 바꿔."),
        ("/최신 소식 목록", "최신 소식 설정과 받는 사람 목록을 개인 메시지로 확인해."),
        ("/최신 소식 중복초기화 확인", "이미 보낸 소식 중복 기록만 비워. 수신자와 설정은 유지돼."),
        ("/주제 목록", "최신 소식 주제를 확인해."),
        ("/주제 추가/제거/설정 [주제]", "최신 소식 주제를 관리해.\n예: `/주제 추가 강화학습`"),
        ("그 긴거 해줘", "샘의 긴 대사를 그대로 출력해."),
        ("/호감도설정 @유저 [숫자]", "특정 사용자의 호감도를 원하는 값으로 설정해.\n예: `/호감도설정 @개척자 75`"),
        ("/호감도증감 @유저 [숫자]", "특정 사용자의 호감도를 올리거나 내려.\n예: `/호감도증감 @개척자 -10`"),
        ("/메모리파일", "현재 저장된 memory.json 파일을 받아와."),
        ("/메모리초기화 확인", "memory.json 전체를 빈 상태로 초기화해.\n`/메모리파일초기화 확인`도 사용할 수 있어."),
