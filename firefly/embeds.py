...
TEAM_HELP_TEXT = (
    ...
    "`/팀나누기 팀별=술래:3,숨는 사람:6 | 철수, 영희, 민수, 수진, 지훈, 하늘, 바다, 노을, 별`"
)
...
def create_help_embed() -> discord.Embed:
    ...
    fields = [
        ...
        ("/가챠", "영호 가챠 사이트 주소를 보여줘."),
        ("/랭킹", "영호 가챠 상위 10명과 내 YP 순위를 보여줘."),
...
def create_special_help_embed() -> discord.Embed:
    ...
    fields = [
        (
            "기본",
            "`/도움말`, `/가챠`, `/랭킹`, `/호감도`, `/초기화`, `/호칭 [이름]`, `/프로필 [@유저]`, `/요약 [개인/방] [개수]`",
        ),
        (
            "도구",
            "`/주사위 1 6`, `/실행 주사위 1 6 && 프로필 @유저 | 결과를 묶어 설명해줘`\n"
            "`/실행 주사위 2 6 | 나온 숫자만큼 투표 항목 만들어줘`\n"
            f"{TEAM_HELP_TEXT}\n"
            "`/검색실행 [프롬프트]`, `/검색실행 [명령어들] || [프롬프트]`\n"
            "`/음성생성 [말할 내용]`, `/음성큐비우기 확인`",
        ),
        (
            "투표",
            f"`/투표`, `/투표마감 [메시지ID/최근]`\n{POLL_HELP_TEXT}",
        ...
        (
            "반디의 뇌",
            "`/뇌 @유저`, `/뇌추가 @유저 [키워드:점수]`, `/뇌수정 @유저 [번호] [키워드:점수]`\n"
            "`/뇌삭제 @유저 [번호]`, `/뇌삭제 @유저 [키워드]`\n"
            "뇌추가는 감정/거리감/신뢰 판단에 쓰는 키워드 점수를 할인률 적용 후 누적해.\n"
        ...
        (
            "메모리",
            "`/메모리파일 [대화/방/투표/뉴스]`, `/메모리 파일`, `/memoryfile`\n"
            "`/메모리초기화 [대상] 확인`, `/메모리파일초기화 [대상] 확인`",
        ),
        (
            "방/봇 관리",
