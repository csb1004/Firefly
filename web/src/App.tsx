...
function DrawPage() {
  ...
  async function draw(count: 1 | 10) {
    ...
    try {
      ...
      setResults(count === 10 ? sortCardsByRarity(cards) : cards);
      setReveal(cards);
      await load();
    }
    catch (e) { setError((e as Error).message); }
...
function CardDetailPage({cardId}:{cardId:string}) { const [card,setCard]=useState<Card>();const [sets,setSets]=useState<SetDefinition[]>([]);useEffect(()=>{Promise.all([api<Card>(`/api/cards/${cardId}`),api<SetDefinition[]>("/api/sets")]).then(([nextCard,nextSets])=>{setCard(nextCard);setSets(nextSets.filter(cardSet=>cardSet.member_cards.some(member=>member.id===cardId)));});},[cardId]);return <section>{card?<><div className="card-detail"><CardTile card={card}/><div><p className="eyebrow">CARD DETAIL</p><h1>{card.name}</h1><Stars rarity={card.rarity}/><strong>{card.yp.toLocaleString()} YP</strong></div></div><section className="card-related-sets"><div className="section-title"><div><p className="eyebrow">RELATED SETS</p><h2>관련 세트 효과</h2></div></div>{sets.length?<SetEffectList sets={sets} progress/>:<div className="empty">이 카드가 구성원으로 포함된 세트가 없습니다.</div>}</section></>:<div className="empty">카드를 불러오는 중…</div>}</section>; }
...
function DrawTicketAdmin() {
  ...
  const [newUserBonusTickets, setNewUserBonusTickets] = useState(0);
  const [query, setQuery] = useState("");
  const [users, setUsers] = useState<User[]>([]);
  const [selected, setSelected] = useState<TicketView>();
  const [grantAmount, setGrantAmount] = useState(1);
  const [message, setMessage] = useState("");
  useEffect(() => { api<{daily_draws:number;new_user_bonus_tickets:number}>("/api/admin/draw-settings").then(data=>{setDailyDraws(data.daily_draws);setNewUserBonusTickets(data.new_user_bonus_tickets);}).catch(e=>setMessage(e.message)); }, []);
  async function saveDaily() { try { const data=await api<{daily_draws:number;new_user_bonus_tickets:number}>("/api/admin/draw-settings",{method:"PUT",body:JSON.stringify({daily_draws:dailyDraws,new_user_bonus_tickets:newUserBonusTickets})},true);setDailyDraws(data.daily_draws);setNewUserBonusTickets(data.new_user_bonus_tickets);setMessage(`매일 ${data.daily_draws}회 · 신규 가입 추가권 ${data.new_user_bonus_tickets}장으로 변경했습니다.`); } catch(e){setMessage((e as Error).message);} }
...
function AdminPage({ onSeasonReset }: { onSeasonReset: (result: SeasonResetResult) => void }) {
  ...
  const [managedCardId, setManagedCardId] = useState<string>();
  ...
  async function edit(card: Card, values: CardEditValues) { const form=new FormData();form.set("name",values.name);form.set("rarity",String(values.rarity));form.set("yp",String(values.yp));if(values.weight)form.set("weight",values.weight);form.set("active",String(card.active));try{await api(`/api/admin/cards/${card.id}`,{method:"PUT",body:form},true);setMessage(`${values.name} 정보를 저장했습니다.`);await load();}catch(e){setMessage((e as Error).message);} }
