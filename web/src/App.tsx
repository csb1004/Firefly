...
function DrawPage() {
  const [status, setStatus] = useState<{ eligible: boolean; four_remaining: number; five_remaining: number }>();
  ...
  return <><section className="hero-panel"><p className="eyebrow">DAILY SIGNAL</p><h1>오늘의 카드</h1><p>매일 오전 5시, 한 번의 신호가 다시 열립니다.</p><div className="pity-grid"><div><span>4성 이상 확정까지</span><b>{status?.four_remaining ?? "–"}회</b></div><div><span>5성 확정까지</span><b>{status?.five_remaining ?? "–"}회</b></div></div><button className="draw-button" disabled={!status?.eligible} onClick={draw}>{status?.eligible ? "신호 연결하기" : "오늘의 뽑기 완료"}</button>{error && <p className="error">{error}</p>}</section>
