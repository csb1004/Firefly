...
function DrawPage() {
  ...
  async function draw() {
    setError("");
    try { const data = await api<any>("/api/draw", { method: "POST", body: JSON.stringify({ idempotency_key: idempotencyKey("draw") }) }, true); setResult(data.card); await load(); }
    catch (e) { setError((e as Error).message); }
  }
  return <><section className="hero-panel"><p className="eyebrow">DAILY SIGNAL</p><h1>오늘의 카드</h1><p>일일 뽑기는 오전 5시에 다시 채워집니다.</p><div className="ticket-balance"><span>오늘 남은 횟수 <b>{status?.daily_remaining ?? "–"}</b></span><span>추가 뽑기권 <b>{status?.bonus_tickets ?? "–"}</b></span></div><div className="pity-grid"><div><span>4성 이상 확정까지</span><b>{status?.four_remaining ?? "–"}회</b></div><div><span>5성 확정까지</span><b>{status?.five_remaining ?? "–"}회</b></div></div><button className="draw-button" disabled={!status?.eligible} onClick={draw}>{status?.eligible ? `신호 연결하기 · ${status.draws_remaining}회 남음` : "사용 가능한 뽑기권 없음"}</button>{error && <p className="error">{error}</p>}</section>
    ...
    {result && <Reveal card={result} onClose={() => setResult(undefined)} />}</>;
}

function CollectionPage({ userId }: { userId?: number }) {
  const [data, setData] = useState<Collection>();
  const [error, setError] = useState("");
  useEffect(() => { api<Collection>(userId ? `/api/users/${userId}/collection` : "/api/collection/me").then(setData).catch(e => setError(e.message)); }, [userId]);
  return <section><div className="section-title"><div><p className="eyebrow">ARCHIVE</p><h1>컬렉션</h1></div><strong className="yp-total">{data?.total_yp.toLocaleString() ?? 0} YP</strong></div>{error && <p className="error">{error}</p>}<div className="card-grid">{data?.cards.map(card => <CardTile card={card} key={card.id} />)}</div>{data?.cards.length === 0 && <div className="empty">아직 획득한 카드가 없습니다.</div>}</section>;
}
