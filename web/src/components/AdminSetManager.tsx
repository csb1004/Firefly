  ...
  return <section className="admin-set-section">
    ...
    <div className="set-manager-grid">
      ...
      <div className="panel set-editor">
        ...
        <div className="template-row"><span>빠른 템플릿</span><button onClick={() => useTemplate("rarity")}>1성 카드 +50 YP</button><button onClick={() => useTemplate("members")}>세트 카드 +5%</button><button onClick={() => useTemplate("collection")}>전체 카드 +10 YP</button></div>
        <div className="effect-list">{draft.effects.map((effect, index) => <div className="effect-editor" key={index}>
          ...
          <div className="effect-fields">
            <label>적용 횟수 대상<select value={effect.target_scope} onChange={event => updateEffect(index, { target_scope: event.target.value as SetEffect["target_scope"] })}><option value="set_members">세트 구성 카드</option><option value="selected_cards">선택한 카드</option><option value="rarity">특정 성급</option><option value="collection">전체 인벤토리</option></select></label>
            ...
            <label>적용 횟수<select value={effect.count_mode} onChange={event => updateEffect(index, { count_mode: event.target.value as SetEffect["count_mode"] })}><option value="once">한 번</option><option value="distinct">서로 다른 카드마다</option><option value="quantity">보유 장수마다</option></select></label>
            <label>YP 증가 대상<select value={effect.bonus_target_scope} onChange={event => updateEffect(index, { bonus_target_scope: event.target.value as SetEffect["bonus_target_scope"] })}><option value="set_members">세트 구성 카드</option><option value="selected_cards">선택한 카드</option><option value="rarity">특정 성급</option><option value="collection">전체 인벤토리</option></select></label>
            ...
            <label>보너스<select value={effect.bonus_type} onChange={event => updateEffect(index, { bonus_type: event.target.value as SetEffect["bonus_type"] })}><option value="fixed">고정 YP</option><option value="percent">최종 %</option></select></label>
            <label>수치<input type="number" min="0" step={effect.bonus_type === "fixed" ? 1 : .01} value={effect.value} onChange={event => updateEffect(index, { value: Number(event.target.value) })}/></label>
          ...
          <p className="effect-help">적용 횟수로 보너스 수치를 곱한 뒤, 선택한 YP 증가 대상 카드 각각에 적용됩니다.</p>
