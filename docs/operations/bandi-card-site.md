...
- 중복 방지: 플레이 기록과 별개인 내부 작업 ID·완료 결과 영수증은 보존한다. 이는 유실된 요청이 나중에 다시 도착해 새 시즌 데이터를 재삭제하지 않도록 하기 위한 최소 운영 데이터다.
...
6. 확인란에 `영호 가챠 시즌 초기화`를 정확히 입력하고 실행 버튼을 누른다.
7. 완료 알림이 표시될 때까지 Railway 재배포나 스케일 변경을 하지 않는다. 브라우저 연결 오류가 표시되면 같은 화면에서 다시 실행할 수 있다.

초기화 중에는 사이트의 새 변경 요청이 잠시 차단된다. 서버는 PostgreSQL 트랜잭션 advisory lock과 관련 테이블 잠금을 잡고 삭제·재지급·감사 생성을 하나의 트랜잭션으로 처리한다. 어느 단계에서든 실패하면 전부 롤백된다. 브라우저는 실행별 작업 ID를 보관하므로 성공 응답이 유실되어 같은 요청을 다시 보내도 초기화를 반복하지 않고 최초 완료 결과를 돌려준다. 이때 화면에는 새 실행이 아니라 `이전 시즌 초기화 완료 결과`를 확인했다는 문구가 표시된다. 성공 커밋 뒤 접속 중인 브라우저에는 작업 ID가 포함된 새 시즌 이벤트가 전달되어 보류 중인 요청을 정리하고 오래된 거래 화면과 데이터를 새로 불러온다.
...
실패 메시지가 나오면 SQL로 직접 정리하지 않는다. 단순 연결 오류라면 같은 화면의 실행 버튼으로 안전하게 재시도하고, 그 밖의 오류라면 Railway 웹 로그를 확인한 뒤 미리보기로 기존 데이터가 유지됐는지 검증한다. 성공으로 표시됐지만 결과가 예상과 다르면 즉시 사용자 변경 작업을 중지하고, 완료 수량과 로그를 보존한 뒤 확인해 둔 백업에서 복구할지 판단한다.
