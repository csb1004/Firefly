...
def test_base_yp_counts_every_copy_while_set_rule_count_stays_once(web_db):
    with web_db() as db:
        ...
        assert result.fixed_bonus == 150
        assert result.total_yp == 450
        assert result.active_sets == ("중복되지 않는 세트",)


def test_percent_effect_only_increases_qualifying_card_yp(web_db):
    with web_db() as db:
        ...
        one_star = Card(name="1성 카드", rarity=1, yp=100, image_key="cards/one.webp")
        ...
        db.add_all([
            ...
            SetEffect(set_id=card_set.id, target_scope="collection", count_mode="once", bonus_target_scope="set_members", bonus_type="fixed", value=100),
            SetEffect(set_id=card_set.id, target_scope="rarity", target_rarity=1, count_mode="quantity", bonus_target_scope="rarity", bonus_target_rarity=1, bonus_type="percent", value=5, max_count=3),
        ])
        db.commit()

        result = effective_yp(db, user.id)
        assert result.base_yp == 4000
...
def test_count_target_and_bonus_target_are_independent(web_db):
    with web_db() as db:
        user = User(discord_id="target-user", username="target", warning_acknowledged=True)
        first = Card(name="세트 카드 A", rarity=4, yp=100, image_key="cards/a.webp")
        second = Card(name="세트 카드 B", rarity=4, yp=200, image_key="cards/b.webp")
        outside = Card(name="보너스 대상 카드", rarity=1, yp=100, image_key="cards/outside.webp")
        card_set = CardSet(name="대상 한정 세트", active=True)
        db.add_all([user, first, second, outside, card_set])
        db.flush()
        db.add_all([
            Inventory(user_id=user.id, card_id=first.id, quantity=1),
            Inventory(user_id=user.id, card_id=second.id, quantity=1),
            Inventory(user_id=user.id, card_id=outside.id, quantity=3),
            CardSetMember(set_id=card_set.id, card_id=first.id),
            CardSetMember(set_id=card_set.id, card_id=second.id),
            SetEffect(
                set_id=card_set.id,
